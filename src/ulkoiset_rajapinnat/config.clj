...
(def ^:private property-key :ulkoiset-rajapinnat-properties)

(defn ^:private read-config-path-from-args [args]
  (first (filter some?
                 (map (fn [arg]
                        (if (str/starts-with? arg (name property-key))
                          (subs arg (+ 1 (count (name property-key)))) nil)) args))))
  ...
  (if-let [from-args (read-config-path-from-args args)]
    ...
    (if-let [from-env (read-config-path-from-env)]
      (do
        ...
        from-env)
      (throw (Exception. (str (name property-key) " is mandatory! Either give it in args or set env.var!"))))))
