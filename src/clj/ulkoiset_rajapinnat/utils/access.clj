(ns ulkoiset-rajapinnat.utils.access
  (:require [clojure.tools.logging :as log]
            ...
            [org.httpkit.server :refer [as-channel with-channel on-close close]])
...
(defn access-log-with-ticket-check-as-channel [ticket audit-log start-future-fn]
  (fn [request]
    (if ticket
      (let [start-time (System/currentTimeMillis)
            log-when-closed-by-client (fn [status] (when (not= status :server-close)
                                                     (write-access-log start-time (format "Closed by client (%s)" status) request)))]
        (let [maybe-running (atom nil)]
          (as-channel request
                      {:on-receive (fn [_ message]
                                     (log/info (str "Message received: " message)))
                       :on-close   (fn [_ status]
                                     (when-let [running @maybe-running]
                                       (.cancel running true))
                                     (log-when-closed-by-client status))
                       :on-open    (fn [channel]
                                     (go
                                       (try
                                         (let [user (<? (check-ticket-is-valid-and-user-has-required-roles ticket))]
                                           (try
                                             (let [f (start-future-fn user)]
                                               (reset! maybe-running f)
                                               (.whenComplete f (reify BiConsumer (accept [_ r e]
                                                                                    (if e
                                                                                      (handle-exception channel start-time e)
                                                                                      (try
                                                                                        ...
                                                                                        (catch Exception e
                                                                                          (handle-exception channel start-time e))))))))
                                             (catch Exception e (do
                                                                  (log/error "Uncaught exception in request handler!" e)
                                                                  (handle-exception channel start-time e)))
                                             (finally
                                               (audit-log "-"
                                                          (user :personOid)
